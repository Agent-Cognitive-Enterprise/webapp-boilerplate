...
## Current objective
Keep CI stable while continuing cross-stack browser coverage improvements. The latest completed work switched the setup redirect to an effect-driven app-level navigation path and finished the last Node 20-based action upgrade.
...
- Hardened the setup redirect in `frontend/src/App.tsx` by persisting the post-setup redirect marker in `sessionStorage` and driving the `/setup` -> `/login` transition from an effect-backed `navigate("/login")`, so CI/browser rerenders still land on `/login` after initialization.
- Updated remaining deprecated GitHub Actions in `.github/workflows/ci.yml` and `.github/workflows/scorecard.yml`: `actions/setup-node@v5`, `actions/setup-python@v6`, and `actions/upload-artifact@v6`.
