...
## Current objective
Keep protected-route hardening and cookie-auth middleware ordering regression-tested in both in-process and live-server paths, including supported write endpoints and auth session endpoints.
...
- Ran focused runtime smoke coverage successfully (`1` targeted test passed after the logout extension).
- Re-ran full backend verification successfully (`240` tests passed, Ruff clean, mypy clean).

## Current status
Protected backend routes now have both an in-process regression matrix and a live-server smoke test. Runtime coverage verifies an admin-denied write path, a supported `/user-settings` write path, and cookie-authenticated logout, so trusted vs untrusted `Origin` behavior is exercised on real HTTP requests across the main cookie-auth middleware paths.

## Next step
Add a live-server smoke test for cookie-authenticated `POST /auth/refresh` with trusted and untrusted `Origin` headers, so runtime CSRF handling is verified on the remaining state-changing auth endpoint that rotates session credentials.
...
The runtime smoke now includes unsafe cookie-authenticated checks for `/admin/settings`, `/user-settings`, and `/auth/logout`. Additional unsafe runtime cases should keep trusted-origin and untrusted-origin expectations explicit so CSRF failures are not mistaken for auth regressions.
