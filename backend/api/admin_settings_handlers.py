...
from services.system_settings import get_system_settings_row, masked, normalize_locales
...
async def get_admin_settings_handler(
    *,
    request: Request,
    ...
    return build_admin_settings_response(
        settings=settings,
...
async def update_admin_settings_handler(
    *,
    ...
    if payload.site_logo is not None:
        settings.site_logo = normalize_optional_text(payload.site_logo)
    if payload.background_image is not None:
        settings.background_image = normalize_optional_text(payload.background_image)
    if payload.openai_api_key is not None:
        settings.openai_api_key = normalize_optional_text(payload.openai_api_key)
    if payload.deepseek_api_key is not None:
        settings.deepseek_api_key = normalize_optional_text(payload.deepseek_api_key)
    ...
    if payload.smtp_password is not None:
        settings.smtp_password = normalize_optional_text(payload.smtp_password)
    ...
    if payload.auth_frontend_base_url is not None:
        settings.auth_frontend_base_url = normalize_optional_text(payload.auth_frontend_base_url)
    if payload.auth_backend_base_url is not None:
        settings.auth_backend_base_url = normalize_optional_text(payload.auth_backend_base_url)
...
async def check_admin_email_settings_handler(
    *,
